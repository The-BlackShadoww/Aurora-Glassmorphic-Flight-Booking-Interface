        ...
        <GlassCard className="p-8 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-[repeat(7,auto)] gap-2 justify-center">
                ...
                {Array.from({ length: ROWS }, (_, rowIndex) => (
                    <>
                        {/* Row Number */}
                        <div key={`row-${rowIndex}`} className="text-white/60 text-sm flex items-center pr-2">
                            {rowIndex + 1}
                        </div>
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {selectedSeats.map(seat => (
                            <span key={seat} className="px-3 py-1 bg-primary-500/30 rounded-lg text-white text-sm">
                                {seat}
                            </span>
                        ))}
                    </div>
